/* PC-88 .grp ->CHR88.BAS •ÏŠ· (‰¡512dot)  for GCC By m@3 */
...
short bload(char *loadfil, char *savefil, unsigned char *buffer1,unsigned char *buffer2, unsigned short size)
{
	...
	for(j = 0; j < LINE; ++j){
		for(i = 0; i < WIDTH; ++i){
			buffer2[(i + j * SIZE * PLANE) ] = buffer1[k + 0];
			buffer2[(i + j * SIZE * PLANE) + SIZE] = buffer1[k + 1];
			buffer2[(i + j * SIZE * PLANE) + SIZE * 2] = buffer1[k + 2];
			k += PLANE;
		}
	}
	for(j = 0; j < LINE; ++j){
		for(i = 0; i < WIDTH; ++i){
			buffer2[(i + j * SIZE * PLANE) + WIDTH] = buffer1[k + 0];
			buffer2[(i + j * SIZE * PLANE) + WIDTH + SIZE] = buffer1[k + 1];
			buffer2[(i + j * SIZE * PLANE) + WIDTH + SIZE * 2] = buffer1[k + 2];
			k += PLANE;
		}
	}
