/* PC-98 BSAVE to Screen CONV.(Analog 16 colors) for GCC-ia16 By m@3 */
/* 全体版 */
...
int conv(char *loadfil, unsigned char __far *flame)
{
	long i;
	int k=0, l=0;
	...
	for(;;){
		size = fread(pattern, 1, BUFFSIZE, stream[0]);
		if(size <= 1)
			break;
		_fmemcpy(flame, pattern, size);
		flame += size;
	}
	fclose(stream[0]);
...
int	main(int argc,char **argv){
	...
	if ((argv[1] == NULL) || (argc < 2)){
		printf("BSAVE to Screen Converter for PC-98.\n");
		return 1;
	}
