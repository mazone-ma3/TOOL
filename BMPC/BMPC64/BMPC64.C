/* Bitmap16(4)colors to C64 SPRITE(C format) Converter By m@3 */
...
long c64width, width, height, filesize, headersize, datasize;

unsigned char read_pattern[C64_WIDTH * C64_HEIGHT + 2];
unsigned char write_pattern[C64_WIDTH * C64_HEIGHT + 2];
unsigned char pattern[10+1];

unsigned char grp_buffer[C64_WIDTH][C64_HEIGHT];

int conv(char *bitmapfil)
{
	...
	c64width = read_pattern[0x0] + 256 * (read_pattern[0x1] + 256 * (read_pattern[0x2]  +  256 * read_pattern[0x3]));
	fread(read_pattern, 1, 4, stream[0]);	/* 縦幅 */
	height = read_pattern[0x0] + 256 * (read_pattern[0x1] + 256 * (read_pattern[0x2]  +  256 * read_pattern[0x3]));
	if((c64width > C64_WIDTH) || (height > (C64_HEIGHT))){
		fclose(stream[0]);
		printf("Size over file %s.", bitmapfil);
		return ERROR;
	}else{
//		printf("X Size %d / Y Size %d", c64width, height);
	}
...
///////////////////////////////////////////////////////////////////////////////
	...
	for(j = 0; j < height; ++j){
		for(i = 0; i < (width / 2); ++i){
//			if((j % 2)){
//			write_pattern[i + (height - j - 1) * (C64_WIDTH / 2)] = read_pattern[k++];
...
///////////////////////////////////////////////////////////////////////////////
	...
	for(j = 0; j < height; ++j){
		printf("\t");
		for(i = 0; i < c64width / 4; ++i){
			data = 0;
			for(m = 0; m < 4; ++m){
				color = grp_buffer[i * 4 + m][j];
				switch(color){
					case 0:
						break;
					case 1:
...
{
	if (argc < 2){
		printf("Bitmap 4colors to C64 SPRITE(C format) Converter.\n");
		return ERROR;
	}
